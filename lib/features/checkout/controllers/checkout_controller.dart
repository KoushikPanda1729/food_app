...
class CheckoutController extends GetxController implements GetxService {
  ...
  List<String> deliveryInstructionList = [
    'Deliver to front door',
    'Deliver to the reception desk',
    'Avoid calling me',
  ];

  void additionalInstruction(String instruction) {
    if(instruction.isNotEmpty) {
      if(deliveryInstructionList.length > 3) {
        deliveryInstructionList.removeAt(3);
      }
      deliveryInstructionList.add(instruction);
    }
    update();
  }
  ...
  Future<void> shanghaiPayment(String? amount, String? storeId) async {
    ...
    if (response.statusCode == 200) {
      ...
      String status = '';
      try{
        redirectUrl = response.body['redirect_link'];
      }catch(e){
        log('catch error shanghaiPayment: $e', name: 'shanghaiPayment');
      ...
      if(status == 'success') {
        _isLoading = false;
        update();
        Get.offNamed(RouteHelper.getOrderSuccessRoute('-1', 'success', null, null, isShanghaiPay: true));
        return;
      }
      ...
      if(GetPlatform.isWeb) {
        ...
        Get.toNamed(RouteHelper.getPaymentRoute(OrderModel(), '', guestId: Get.find<AuthController>().getGuestId(), shanghaiPayUri: redirectUrl));
      }
    }

...
class OrderSuccessfulScreen extends StatefulWidget {
  ...
  final bool isShanghaiPayment;
  const OrderSuccessfulScreen({super.key, required this.orderID, required this.status, required this.totalAmount, this.contactPersonNumber, this.isDeliveryOrder = false, this.isShanghaiPayment = false});
...
class _OrderSuccessfulScreenState extends State<OrderSuccessfulScreen> {
  ...
  void initState() {
    ...
    if(!widget.isShanghaiPayment) Get.find<OrderController>().trackOrder(orderId.toString(), null, false, contactNumber: widget.contactPersonNumber);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<OrderController>(builder: (orderController) {
        ...
        if(widget.isShanghaiPayment) {
          success = widget.status == 1;
        }

        return orderController.trackModel != null || widget.isShanghaiPayment ? Center(child: SingleChildScrollView(
          ...
          child: FooterViewWidget(
            child: SizedBox(width: Dimensions.webMaxWidth, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(
                success ? widget.isShanghaiPayment ? 'your_payment_is_successfully_processed'.tr : 'you_placed_the_order_successfully'.tr :  widget.isShanghaiPayment ? 'your_payment_is_not_successfully_processed'.tr :  'your_order_is_failed_to_place'.tr,
                style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge),
              ),
              const SizedBox(height: Dimensions.paddingSizeSmall),

              Get.find<AuthController>().isGuestLoggedIn() && !widget.isShanghaiPayment ? Text(
                '${'order_id'.tr}: $orderId',
                style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(context).primaryColor),
              ) : const SizedBox(),

              if(!widget.isShanghaiPayment) Padding(
              ...
              !widget.isShanghaiPayment && Get.find<AuthController>().isLoggedIn() && ResponsiveHelper.isDesktop(context) && (success && Get.find<SplashController>().configModel!.loyaltyPointStatus == 1 && total.floor() > 0 )  ? Column(children: [

...
class PaymentScreen extends StatefulWidget {
  ...
  final int? packageId;
  final String? shanghaiPayUri;
  const PaymentScreen({super.key, required this.orderModel, required this.paymentMethod, this.addFundUrl, this.subscriptionUrl,
    required this.guestId, required this.contactNumber, this.restaurantId, this.packageId, this.shanghaiPayUri});
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    super.initState();
    log('shanghai pay uri1 -> ${widget.shanghaiPayUri}', name: 'PaymentScreen');
    if(widget.addFundUrl == '' && widget.addFundUrl!.isEmpty && widget.subscriptionUrl == '' && widget.subscriptionUrl!.isEmpty && (widget.shanghaiPayUri == null || widget.shanghaiPayUri!.isEmpty)) {
    ...
    } else if(widget.shanghaiPayUri?.isNotEmpty ?? false) {
      log('shanghai pay uri -> ${widget.shanghaiPayUri}', name: 'PaymentScreen');
      selectedUrl = widget.shanghaiPayUri!;
    }
    else {
  ...
  void _initData() async {
    ...
    browser = MyInAppBrowser(shanghaiPayUri: widget.shanghaiPayUri, orderID: widget.orderModel.id.toString(), orderAmount: widget.orderModel.orderAmount, maxCodOrderAmount: maxCodOrderAmount, addFundUrl: widget.addFundUrl,
...
class MyInAppBrowser extends InAppBrowser {
  ...
  MyInAppBrowser({required this.orderID, required this.orderAmount, required this.maxCodOrderAmount, this.contactNumber, super.windowId,
    super.initialUserScripts, this.addFundUrl, this.subscriptionUrl, this.restaurantId, this.packageId, this.isDeliveryOrder = false, this.shanghaiPayUri});
  ...
  Future onLoadStart(url) async {
    ...
    _redirect(url.toString(), contactNumber, restaurantId, packageId, shanghaiPayUri != null && shanghaiPayUri!.isNotEmpty);
  }
  ...
  @override
  void onExit() {
    if(_canRedirect && (shanghaiPayUri == null || shanghaiPayUri!.isEmpty)) {
  ...
  void _redirect(String url, String? contactNumber, int? restaurantId, int? packageId, bool? isShanghaiPay) {
    ...
    if(_canRedirect) {
      ...
      log('isSuccess -> $isSuccess', name: 'PaymentScreen');
      ...
      if((addFundUrl == '' && addFundUrl!.isEmpty && subscriptionUrl == '' && subscriptionUrl!.isEmpty)){
        _orderPaymentDoneDecision(isSuccess, isFailed, isCancel, isShanghaiPay!);
      } else{
  ...
  void _orderPaymentDoneDecision(bool isSuccess, bool isFailed, bool isCancel, bool isShanghaiPay) {
    if (isSuccess) {
      if(!isShanghaiPay) {
      ...
      Get.offNamed(RouteHelper.getOrderSuccessRoute(orderID, 'success', orderAmount, contactNumber, isDeliveryOrder: isDeliveryOrder, isShanghaiPay: isShanghaiPay));
    } else if (isFailed || isCancel) {
      Get.offNamed(RouteHelper.getOrderSuccessRoute(orderID, 'fail', orderAmount, contactNumber, isDeliveryOrder: isDeliveryOrder, isShanghaiPay: isShanghaiPay));
    }
  }

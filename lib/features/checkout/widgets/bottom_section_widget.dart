        ...
        child: ExpansionTile(
          ...
          children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if(packagingPrice > 0) Text('packaging_charge_for'.tr, style: robotoRegular),
              if(packagingPrice > 0) const SizedBox(height: Dimensions.paddingSizeExtraSmall),
              if(cartList != null) Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: cartList!.map<Widget>((cart) {
                  final packagingCharge = (cart.product?.category?.packagingPrice ?? 0) * cart.quantity!;
                  if (packagingCharge == 0) return const SizedBox.shrink();
                  return Padding(
                    padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeExtraSmall),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(' ${cart.product?.category?.name ?? 'no name'} (${cart.quantity} ${'items'.tr} * ${PriceConverter.convertPrice(cart.product?.category?.packagingPrice ?? 0)})', style: robotoRegular),
                        Text(
                          '(+) ${PriceConverter.convertPrice(packagingCharge.toDouble())}',
                          style: robotoRegular,

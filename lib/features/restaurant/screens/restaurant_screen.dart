...
import 'package:stackfood_multivendor/helper/store_helper.dart';
    ...
    return Scaffold(
      ...
      floatingActionButton: GetBuilder<RestaurantController>(
        builder: (restController) {
          return (restController.restaurant != null && restController.restaurant!.name != null) ? InkWell(
            onTap: () {
              log('Restaurant ID: ${restController.restaurant?.id}');

              if(!StoreHelper.inZone(restController.restaurant)) {
                showCustomSnackBar('this_not_available_in_your_area'.tr, isError: true);
                return;
              }
              Get.toNamed(RouteHelper.getPayRestaurantRoute(
                restaurant: restController.restaurant,
              ));
            },
            child: Container(
              ...
              padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
              child: Text('pay_restaurant'.tr, style: robotoMedium.copyWith(

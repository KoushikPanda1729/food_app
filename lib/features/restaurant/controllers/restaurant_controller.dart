...
//       getRestaurantList(1, false, restaurant,
...
class RestaurantController extends GetxController implements GetxService {
  ...
  /// ─── 2) getRestaurantList no longer takes a “String? restaurant” parameter ───────────────────
  Future<void> getRestaurantList(int offset, bool reload,
    ...
    // Always read the filter from `_filterName`
    final filter = _filterName;
    ...
    if (source == DataSourceEnum.local && offset == 1) {
      restaurantModel = await restaurantServiceInterface.getRestaurantList(
          ...
          _veg,
          _nonVeg,
          filter, // ← use stored filterName here
          fromMap: fromMap,
          source: DataSourceEnum.local);
      _prepareRestaurantList(restaurantModel, offset);
  ...
  /// ─── 3) setRestaurantType, setTopRated, setDiscount, setVeg, setNonVeg → no “currentRestaurant” needed ───
